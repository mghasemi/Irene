---
description: "Run a multi-source research workflow by orchestrating local skills (LightRAG, ZIMI, NotebookLM, SearXNG) and return a grounded synthesis."
...
2. If project-context grounding is needed, query NotebookLM wrapper:
   - `bash scripts/notebooklm_py.sh status`
   - `bash scripts/notebooklm_py.sh ask "{{question}}"`
3. If recency or web corroboration is needed, run SearXNG:

"""A module for working with commutative semigroups and their algebras.

This module provides classes and functions for working with commutative semigroups and their algebras.
...
def _degree(xprsn):
    ...
    Args:
        xprsn (sympy.Expr): The expression whose degree is to be computed.
...
class CommutativeSemigroup(object):
    ...
    def __init__(self, gens, is_semigroup=True, is_abelian=True):
    ...
    def _reduce(self, xprsn, recur=True):
    ...
    def degree(self, expr):
        ...
        Args:
            expr (sympy.Expr): The expression whose degree is to be computed.
    ...
    def element_sub_lattice(self, elm, ex=set([])):
...
class AtomicSGElement(object):
    ...
    def __init__(self, semigroup, element):
        ...
        if not isinstance(semigroup, CommutativeSemigroup):
            raise TypeError("'semigroup' should be an insyance of 'CommutativeSemigroup'.")
    ...
    def __pow__(self, p):
        content = [(self.content[0][0], self.content[0][1] ** p)]
        return SemigroupAlgebraElement(content, self.semigroup)
    ...
    def __eq__(self, other):
        M = self.content[0][1]
        if isinstance(other, (int, float)):
            if M is self.semigroup.G.identity:
                return True
        elif isinstance(other, AtomicSGElement):
            return M == other.content[0][1]
        elif isinstance(other, SemigroupAlgebraElement):
            return M == other._max_content()
        return False
...
class SemigroupAlgebraElement(object):
    ...
    def __init__(self, terms, semigroup):
    ...
    def _max_content(self):
        ...
        """
        M = max([_[1] for _ in self.content])
        return M
    ...
    def __add__(self, other):
        ...
        if isinstance(other, (int, float)):
            ...
            content = [(other.content[0][0] + _[0] if other.content[0][1] == _[1] else _[0], _[1]) for _ in
                       self.content]
            content = []
            temp_keys = []
            for _ in self.content:
        ...
        if isinstance(expr, (int, float)):
            ...
            for trm in expr.content:
                ...
                if not sprt:
                    ...
                    res = res + cf * (exp * base_map[sym_chr] * symb ** (exp - 1) * rest + symb ** exp * self.diff(rest,
                                                                                                                   base_map))
        return res

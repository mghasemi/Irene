...
class CommutativeSemigroup(object):
    ...
    def lattice_edges(self, degree: int):
    ...
    def element_sub_lattice(self, elm: Expr, ex: set = set([])) -> set:
...
class AtomicSGElement(object):
    ...
    def lt_divisible_by(self, expr) -> bool:
...
class SemigroupAlgebra(object):
    ...
    def __init__(self, semigroup):
        ...
        self.one = SemigroupAlgebraElement([(0., self.semigroup.G.identity)], self.semigroup)
    ...
    def add_derivative(self, base_map: dict):
    ...
    def diff(self, expr, base_map: dict):
        """Computes the derivative of an expression in the algebra.

...
- In progress:
	- Transition from pilot evidence to manuscript-grade synthesis of confirmed, weakened, and negative claims.
- Next execution sprint:
	1. Fold the new TeX tables into the manuscript narrative and add a reproducibility appendix fragment tied to the existing $d=4,d=5$ artifacts.
	2. Keep the $d=6$ clean/diagnostic slice deferred until the manuscript pass is stabilized; preserve only a placeholder in the current tables.
	3. Revisit cross-degree extension after the manuscript-integrated $d=4,d=5$ evidence is frozen.

...
if __name__ == '__main__':
    ...
    fun = lambda x: -(np.sin(x[0]) - 1)**3 - (np.sin(x[0]) -
                                           np.cos(x[1]))**4 - (np.cos(x[1]) - 3)**2
    cons = (
        {'type': 'ineq', 'fun': lambda x: 10 - (np.sin(x[0]) - 1)**2},
        {'type': 'ineq', 'fun': lambda x: 10 - (np.sin(x[0]) - np.cos(x[1]))**2},
        {'type': 'ineq', 'fun': lambda x: 10 - (np.cos(x[1]) - 3)**2})
